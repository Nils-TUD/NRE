...
#    include "reg.h"
    ...
    void prioritize_events(CpuMessage &msg) {
        ...
        if((cpu->intr_state & 0x3) || (~cpu->efl & 0x200) || cpu->actv_state == 2)
            return;
    ...
    bool receive(CpuMessage &msg) {
        ...
        switch(msg.type) {
            ...
            case CpuMessage::TYPE_CALC_IRQWINDOW: {
                ...
                if(new_event & EVENT_INTR)
                    msg.cpu->inj_info |= INJ_IRQWIN;
                if(new_event & EVENT_NMI)
                    msg.cpu->inj_info |= INJ_NMIWIN;
                return true;
            }
