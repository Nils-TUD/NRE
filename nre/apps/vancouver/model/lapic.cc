...
private:
	...
	bool x2apic_mode() {
		return (_msr & 0xc00) == 0xc00;
	}
	unsigned x2apic_ldr() {
		return ((_initial_apic_id & ~0xf) << 12) | (1 << (_initial_apic_id & 0xf));
	}
	...
	bool set_base_msr(uint64_t value) {
		...
		if((value & ~mask) || (value & 0xc00) == 0x400 || (was_x2apic_mode && (value & 0xc00) == 0x800))
			return false;
	...
	bool send_ipi(unsigned icr,unsigned dst) {
		...
		if(event == VCVCpu::EVENT_LOWEST && x2apic_mode())
			return set_error(4);
	...
	unsigned prioritize_irq() {
		...
		if(irrv && (irrv & 0xf0) > processor_prio())
			return irrv;
		return 0;
	}
	...
	void broadcast_eoi(unsigned vector) {
		...
		if(vector == (_LINT0 & 0xff))
			_rirr[_LINT0_offset - LVT_BASE] = false;
		if(vector == (_LINT1 & 0xff))
			_rirr[_LINT1_offset - LVT_BASE] = false;
	...
	bool register_read(unsigned offset,unsigned &value) {
		...
		if(in_range(offset,LVT_BASE,NUM_LVT)) {
			...
			if(_lvtds[offset - LVT_BASE])
				value |= 1 << 12;
			if(_rirr[offset - LVT_BASE])
				value |= MessageApic::ICR_ASSERT;
		}
		return res;
	}
		...
		switch(offset) {
			...
			case 0xb: // EOI
				...
				if(_isrv) {
					...
					if(_isrv == (_TIMER & 0xff))
						update_timer(_mb.clock().source_time());
...
public:
	...
	bool receive(MessageLegacy &msg) {
		...
		if(msg.type == MessageLegacy::INTR) {
			...
			if(!hw_disabled())
				trigger_lvt(_LINT0_offset - LVT_BASE);
		}
