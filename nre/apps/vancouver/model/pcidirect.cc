...
public:
    void read_all_bars(unsigned bdf, unsigned long *base, unsigned long *size) {
        ...
        for(unsigned i = 0; i < count_bars(bdf); i++) {
            ...
            if((old & 7) == 0x4) {
                if(conf_read(bdf, BAR0 + i + 1)) Logging::panic("64bit bar %x:%8x with high bits set!",
                                                                conf_read(bdf,
                                                                          BAR0 + i + 1), old);
                i++;
            }
...
private:
    ...
    void map_bars(unsigned long *bases, unsigned long *sizes) {
        for(unsigned i = 0; i < _bar_count; i++) {
            ...
            if((bases[i] & 1) == 1) {
                ...
                MessageHostOp msg(MessageHostOp::OP_ALLOC_IOIO_REGION,
                                  (_barinfo[i].port << 8) |  Cpu::bsr(
                                      sizes[i] | 0x3));
                ...
                MessageHostOp msg(MessageHostOp::OP_ALLOC_IOMEM, bases[i] & ~0x1f, 1 << Cpu::bsr(
                                      ((sizes[i] - 1) | 0xfff) + 1));
    ...
    bool match_iobars(unsigned short port, unsigned short &newport, unsigned size) {
        ...
        for(unsigned i = 0; i < _bar_count; i++) {
            if(!_barinfo[i].io ||
               !in_range(port, _cfgspace[BAR0 + i] & BAR_IO_MASK, _barinfo[i].size - size + 1)) continue;
    ...
    unsigned match_bars(unsigned long address, unsigned size, unsigned *&ptr) {
        ...
        for(unsigned i = 0; i < _bar_count; i++) {
            ...
            if(_barinfo[i].size < size || _barinfo[i].io ||
               !in_range(address, _cfgspace[BAR0 + i] & BAR_MEM_MASK,
                         _barinfo[i].size - size + 1))
                continue;
            ptr =
                reinterpret_cast<unsigned *>(_barinfo[i].ptr + address -
                                             (_cfgspace[BAR0 + i] & BAR_MEM_MASK));
            if(_msix_host_table && ptr >= reinterpret_cast<unsigned *>(_msix_host_table) && ptr <
               reinterpret_cast<unsigned *>(_msix_host_table + _irq_count))
                ptr = reinterpret_cast<unsigned *>(_msix_table) +
                      (ptr - reinterpret_cast<unsigned *>(_msix_host_table));
            return BAR0 + i;
        }
        return 0;
    }
...
public:
    ...
    bool receive(MessageMem &msg)
    {
        ...
        if(msg.read) {
            ...
            *ptr = *msg.ptr;
            ...
            if(_msix_host_table && ptr >= reinterpret_cast<unsigned *>(_msix_table) && ptr <
               reinterpret_cast<unsigned *>(_msix_table + _irq_count) && (msg.phys & 0xf) == 0xc) {
                _msix_host_table[(ptr -
                                  reinterpret_cast<unsigned *>(_msix_table)) / 16].control = *msg.ptr;
    ...
    bool  receive(MessageMemRegion &msg) {
        for(unsigned i = 0; i < _bar_count; i++) {
            if(_barinfo[i].io || !_barinfo[i].size ||
               !in_range(msg.page, _cfgspace[BAR0 + i] >> 12, _barinfo[i].size >> 12)) continue;
            ...
            Logging::printf(" MAP %d %lx+%x from %p size %lx page %lx %x\n", i, msg.start_page << 12,
                            msg.count << 12, msg.ptr, _barinfo[i].size, msg.page,
                            _cfgspace[BAR0 + i]);
            return true;
        }
        return false;
    ...
    bool  receive(MessageLegacy &msg) {
        ...
        Logging::printf("disabled DMA by PCI device %x cmd %x -> %x\n", _hostbdf, old,
                        conf_read(_hostbdf, 1));
        return true;
    }



    DirectPciDevice(Motherboard &mb, unsigned hbdf, unsigned guestbdf, bool assign, bool use_irqs = true,
                    unsigned parent_bdf = 0, unsigned vf_no = 0,
                    bool map = true)
        : HostVfPci(mb.bus_hwpcicfg,
                    mb.bus_hostop), _mb(mb), _hostbdf(hbdf), _msix_table(0), _msix_host_table(0),
          _bar_count(count_bars(_hostbdf))
    {
        ...
        check0(assign && !mb.bus_hostop.send(
                   msg4), "DPCI: could not directly assign %x via iommu", _hostbdf);
        ...
        if(_msix_cap) {
            ...
            _msix_host_table =
                reinterpret_cast<MsiXTableEntry *>(_barinfo[_msix_bar].ptr +
                                                   (_cfgspace[1 + _msix_cap] & ~0x7));
            // disable MSIX
