...
class Rtc146818 : public StaticReceiver<Rtc146818> {
	...
	timevalue get_counter() {
		timevalue value = _clock.time(1 << 30);
	...
	unsigned update_cycle(timevalue now) {
		...
		if((~_ram[0xb] & 0x80) && seconds != (_last / FREQ)) {
			...
			update_ram(seconds);
			if(next_alarm(last_seconds) <= seconds)
				set_irqflags(_ram[0xc] | 0x20);
		}
		_last = now;
		return fnow;
	}
	...
	void update_timer(timevalue last_seconds,timevalue now) {
		...
		if((_ram[0xb] & 0x40) && periodic_tics)
			...
			if(alarm != ~0ull)
				next = (alarm - last_seconds) * FREQ - now % FREQ;
		}
		if(next) {
		...
		if(msg.port & 1) {
			...
			switch(_index) {
				...
				case 0xa:
					msg.value &= 0x7f;
					// XXX what if SET=1?
					if((~_ram[0xb] & 0x80) && mod >= (FREQ - tBUC - tUC))
						msg.value |= 0x80;
					break;
				case 0xc:
					set_irqflags(0);
			...
			switch(_index) {
				...
				case 0xa: {
					...
					if(toggled_reset) {
						...
						_offset = _clock.time(FREQ) - FREQ / 2;
