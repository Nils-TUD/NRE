...
private:
	...
	unsigned char get_iir() {
		...
		if((_regs[IER] & 8) && (_regs[MSR] & 0xf))
			value = 0;
		if((_regs[IER] & 2) && (_regs[LSR] & 0x20))
			value = 2;
		if((_regs[IER] & 1) && (((~_regs[FCR] & 1) && (_regs[LSR] & 1)) || (_triggerlevel <= _rfcount)))
			value = 4;
		if((_regs[IER] & 4) && (_regs[LSR] & 0x1e))
			value = 6;
		if(_regs[FCR] & 1)
			value |= 0xc;
		return value;
