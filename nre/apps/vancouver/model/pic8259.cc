...
class PicDevice: public StaticReceiver<PicDevice> {
	...
	// helper functions
	bool is_slave() {
		return (_icw[ICW4] & ICW4_BUF) ? (~_icw[ICW4] & ICW4_MS) : _virq;
	}
	void rotate_prios() {
		_prio_lowest = (_prio_lowest + 1) & 7;
	}
...
public:
	...
	bool receive(MessageLegacy &msg) {
		if(msg.type != MessageLegacy::INTA)
			return false;
		unsigned char vec;
		get_irqvector(vec);
		msg.value = vec;
	...
	bool receive(MessageIOOut &msg) {
		...
		if(msg.port == _elcr_base)
			...
			else if((msg.value & 0x08) == 0) {
				if(msg.value & 0x40) {
					if(msg.value & 0x20)
						specific_eoi(1 << (msg.value & 7));
					if(msg.value & 0x80)
						_prio_lowest = msg.value & 7;
				}
