...
class PitCounter : public StaticReceiver<PitCounter> {
	...
	unsigned short get_counter() {
		...
		long long res = _start - _clock.time(FREQ);
		if(_modus & BCD)
			res = res % 10000;

		// are we still having an old value?
		if(_start - _initial - 1 == _clock.time(FREQ))
			return _latch;

		if(res <= 0)
			load_counter();
		if(feature(FPERIODIC) && (res <= 0))
			res = _initial + (res % _initial);
		if(feature(FSQUARE_WAVE))
			res = ((res * 2 % (_initial + 1)) + (~_initial & 1)) & ~1;
		if(_modus & BCD)
			res = 10000 + (res % 10000);
		return res;
	}
	...
	void reload_counter() {
		...
		_stopped = 0;
		_start = _clock.time(FREQ) + _new_counter + 1;
		load_counter();
		update_timer();
	}
...
public:
	...
	void set_gate(unsigned char value) {
		...
		if(!value && feature(FGATE_DISABLE_COUNTING))
			disable_counting();
		if(value) {
			...
			if(!feature(Features(FSOFTWARE_TRIGGER)))
				...
				_start = _clock.time(FREQ) + _initial + 1;
				_stopped = 0;
			}
		}
	}
	 ...
	 */
	bool get_out() {
		if(_stopped || _start - _initial - 1 == _clock.time(FREQ))
			return _stopped_out;

		if(feature(FCOUNTDOWN))
			return _clock.time(FREQ) >= _start;
		if(feature(FPERIODIC)) {
			if(!feature(FSQUARE_WAVE))
				return get_counter() != 1;
			else
				return ((_clock.time(FREQ) - _start + _initial) % _initial) * 2 < _initial;
		}
		return _clock.time(FREQ) != _start;
	}
	...
	void write(unsigned char value) {
		if(_wrote_low) {
			...
			if(_modus & RW_HIGH) {
				_wrote_low = 1;
				if(feature(Features(FSOFTWARE_TRIGGER | FCOUNTDOWN)) && !get_out())
					disable_counting();
			}
		...
		else if(feature(FPERIODIC) && _gate) {
			if(_stopped)
				reload_counter();
			else
				_start = _clock.time(FREQ) + get_counter();
		}
	}
