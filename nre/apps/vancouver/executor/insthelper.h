...
#define MOVE2(operand_size, DST, SRC) {					\
  ...
  static void move(void * tmp_dst, void *tmp_src, unsigned order) { cpu_move(tmp_dst,tmp_src,order); }
  template<unsigned operand_size> static void move(void *tmp_dst, void *tmp_src) { cpu_move<operand_size>(tmp_dst,tmp_src); }
