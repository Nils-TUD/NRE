...
int handle_segment(CpuState::Descriptor *desc,unsigned &virt,unsigned length,bool write,bool stackop) {
	...
	else if(_entry->address_size == 1)
		virt &= 0xffff;
	if(!(~desc->limit) && !desc->base && desc->ar == 0xc93)
		return 0;
	...
	if((desc->ar & 0x40c) == 4)
		fault |= (0xffff - length) < virt;

	// rights check: present, readonly and exec-only segment
	fault |= ~desc->ar & 0x80 || !write && ((desc->ar & 0xa) == 0x8)
			|| write && (desc->ar & 0xa) != 0x2;
	if(fault) {
		nre::Serial::get().writef("%s() #%x %x+%d desc %x limit %x base %x esp %x\n",__func__,
				desc - &_cpu->es,virt,length,desc->ar,desc->limit,desc->base,_cpu->esp);
		if(desc == &_cpu->ss) {
			SS0;
		}
		else {
...
static void move(void * tmp_dst,void *tmp_src,unsigned order) {
	cpu_move(tmp_dst,tmp_src,order);
}
template<unsigned operand_size> static void move(void *tmp_dst,void *tmp_src) {
	cpu_move<operand_size>(tmp_dst,tmp_src);
}
...
int helper_HLT() {
	return send_message(CpuMessage::TYPE_HLT);
}
int helper_WBINVD() {
	return send_message(CpuMessage::TYPE_WBINVD);
}
int helper_INVD() {
	return send_message(CpuMessage::TYPE_INVD);
}
int helper_CLTS() {
	_cpu->cr0 &= ~(1 << 3);
	return _fault;
...
int helper_INT3() {
	_oeip = _cpu->eip;
	_mtr_out |= nre::Mtd::RIP_LEN;
	return _fault = 0x80000603;
}
int helper_UD2A() {
	return _fault = 0x80000606;
}
int helper_INTO() {
	_oeip = _cpu->eip;
	_mtr_out |= nre::Mtd::RIP_LEN;
...
void __attribute__((regparm(3))) helper_RET(void *tmp_src) {
	...
	if(tmp_src)
		_cpu->esp += *reinterpret_cast<unsigned short *>(tmp_src);
}
...
void __attribute__((regparm(3))) helper_LRET(void *tmp_src) {
	...
	helper_POP<operand_size>(&tmp_eip) || helper_POP<operand_size>(&tmp_cs)
			|| helper_far_jmp(tmp_cs,tmp_eip,_cpu->efl);
	if(tmp_src)
		_cpu->esp += *reinterpret_cast<unsigned short *>(tmp_src);
}
...
#define helper_LOOPS(NAME, X)																	\
	...
	{																							\
		...
		if (X != 3) --ecx;																		\
		if ((ecx && (X == 0 || (X == 1 && (_cpu->efl & 0x40)) || 								\
				(X == 2 && (~_cpu->efl & 0x40)))) || (!ecx && X == 3)) {						\
...
int __attribute__((regparm(3))) string_helper() {
	while(_entry->address_size == 1 && _cpu->cx || _entry->address_size == 2 && _cpu->ecx
			|| !(_entry->prefixes & 0xff)) {
		void *src = &_cpu->eax;
		void *dst = &_cpu->eax;

		FEATURE(SH_LOAD_ESI,
				NCHECK(logical_mem<operand_size>((&_cpu->es) + ((_entry->prefixes >> 8) & 0xf), _cpu->esi, false, src)));
		...
		if(_cpu->efl & 0x400)
			size = -size;
		FEATURE(SH_LOAD_ESI,
				if (_entry->address_size == 1) _cpu->si += size; else _cpu->esi += size;);
		FEATURE(SH_LOAD_EDI | SH_SAVE_EDI,
				if (_entry->address_size == 1) _cpu->di += size; else _cpu->edi += size;);
		if(!(_entry->prefixes & 0xff))
			break;
		if(_entry->address_size == 1)
			_cpu->cx--;
		else
			_cpu->ecx--;
...
int helper_MOV__CR0__EDX() {
	word_t *tmp_src;
	word_t *tmp_dst = get_reg32(_entry->data[_entry->offset_opcode] & 0x7);
...
int helper_MOV__EDX__CR0() {
	word_t *tmp_src = get_reg32(_entry->data[_entry->offset_opcode] & 0x7);
	word_t *tmp_dst;
	word_t tmp = *tmp_src;
...
int helper_LMSW(unsigned short value) {
	_cpu->cr0 = _cpu->cr0 & ~0xeu | value & 0xfu;
	_mtr_out |= nre::Mtd::CR;
	return _fault;
}
...
struct Descriptor {
	...
	unsigned offset() {
		return (values[1] & 0xffff0000) | (values[0] & ~0xffff0000);
	}
	unsigned limit() {
...
int load_idt_descriptor(Descriptor &desc,unsigned event) {
	...
	if(!prepare_virtual(_cpu->id.base + ofs,8,MemTlb::TYPE_R,res)) {
		...
		if(!(0xce00 & (1 << (desc.ar0 & 0x1f)))) {
			nre::Util::panic("%s event %x %x base %x limit %x cr0 %x\n",__func__,event,desc.ar0,
					_cpu->id.base,_cpu->id.limit,_cpu->cr0);
			GP(error);
		}
		// and present?
		if(~desc.ar0 & 0x80)
...
int helper_LTR(unsigned short selector) {
	...
	if(!load_gdt_descriptor(desc,selector,false)) {
		if((desc.ar0 & 0x1f) != 0x9 && (desc.ar0 & 0x1f) != 0x1)
			GP(selector);
		if(~desc.ar0 & 0x80)
			NP(selector);
...
int set_segment(CpuState::Descriptor *seg,unsigned short sel,bool cplcheck = true) {
	if(!_cpu->pm() || _cpu->v86())
		...
		if(!load_gdt_descriptor(desc,sel,false)) {
			if((is_ss
					&& ((rpl != desc.dpl() || cplcheck && desc.dpl() != _cpu->cpl())
							|| ((desc.ar0 & 0x1a) != 0x12)))
					|| !is_ss
							&& ((((desc.ar0 ^ 0x12) & 0x1a) > 2)
									|| (((desc.ar0 & 0xc) != 0xc)
											&& (rpl > desc.dpl()
													|| cplcheck && _cpu->cpl() > desc.dpl())))) {
				nre::Serial::get().writef(
						"set_segment %x sel %x eip %x efl %x ar %x dpl %x rpl %x cpl %x\n",
						seg - &_cpu->es,sel,_cpu->eip,_cpu->efl,desc.ar0,desc.dpl(),rpl,
						_cpu->cpl());
				GP(sel);
			}
			if(~desc.ar0 & 0x80)
...
int helper_far_jmp(unsigned tmp_cs,unsigned tmp_eip,unsigned tmp_flag) {
	...
	// realmode + v86mode
	if(!_cpu->pm() || _cpu->v86()) {
		if(tmp_eip > _cpu->cs.limit)
			GP0;
		_cpu->cs.ar = (_cpu->cs.ar & ~0xf) | 0x3;
		set_realmode_segment(&_cpu->cs,tmp_cs,_cpu->v86());
		_cpu->eip = tmp_eip;
		_cpu->efl = (_cpu->efl & (EFL_VIP | EFL_VIF | EFL_VM | EFL_IOPL))
				| (tmp_flag & ~(EFL_VIP | EFL_VIF | EFL_VM | EFL_IOPL));
	}
	else {
		Descriptor desc;
		if(!load_gdt_descriptor(desc,tmp_cs,false)) {
			if(tmp_eip > desc.limit())
				GP0;
...
int helper_lcall(void *tmp_src) {
	...
	else if(modrm2mem(addr,2 + (1 << operand_size),user_access(TYPE_R)))
		return _fault;
	...
	if(lcall && (helper_PUSH<operand_size>(&cs_sel) || helper_PUSH<operand_size>(&_cpu->eip)))
		return _fault;
...
int helper_IRET() {
	...
	else if(_cpu->pm() && _cpu->efl & (1 << 14))
		UNIMPLEMENTED(this); // task return

	// protected mode
	unsigned tmp_eip = 0,tmp_cs = 0,tmp_flag = _cpu->efl;
	if(helper_POP<operand_size>(&tmp_eip) || helper_POP<operand_size>(&tmp_cs)
			|| helper_POP<operand_size>(&tmp_flag))
		return _fault;
	...
	if((tmp_flag & EFL_VM) && !_cpu->cpl()) {
		...
		for(unsigned i = 0; i < 4; i++)
			if(helper_POP<operand_size>(sels + i))
				return _fault;
		...
		if(!load_gdt_descriptor(desc,tmp_cs,false)) {
			...
			if((tmp_cs & 3) != _cpu->cpl()) {
				...
				if(helper_POP<operand_size>(&tmp_esp) || helper_POP<operand_size>(&tmp_ss))
					return _fault;
				_cpu->esp = tmp_esp;
				if(set_segment(&_cpu->ss,tmp_ss,false))
					return _fault;
			}
...
int idt_traversal(unsigned event,unsigned error_code) {
	...
	if(!_cpu->pm()) {
		...
		unsigned idt;
		if(prepare_virtual(_cpu->id.base + ofs,4,MemTlb::TYPE_R,res))
			return _fault;
		move<2>(&idt,res);
		if(helper_PUSH<1>(&_cpu->efl) || helper_PUSH<1>(&_cpu->cs.sel)
				|| helper_PUSH<1>(&_cpu->eip))
			return _fault;
		if(_cpu->id.limit < (ofs | 3))
			GP0;
	...
	if(!load_idt_descriptor(idt,event)) {
		if((idt.ar0 & 0x1f) == 0x05)
			UNIMPLEMENTED(this); // task gate
		bool ext = (event & 0x700) <= 0x300;
		Descriptor desc;
		...
		switch(idt.ar0 & 0x1f) {
			...
			{
				if(load_gdt_descriptor(desc,idt.base0,ext))
					return _fault;
				if(((desc.ar0 & 0x18) != 0x18) || (desc.dpl() > _cpu->cpl()))
					GP(idt.base0 | ext);
				if(~desc.ar0 & 0x80)
					NP(idt.base0 | ext);
				...
				nre::Serial::get().writef("IDT ar %x dpl %x cpl %x sel %x vec %x eip %x/%x/%x\n",
						desc.ar0,desc.dpl(),_cpu->cpl(),idt.base0,event,_cpu->eip,_cpu->cs.sel,
						old_efl);
				...
				if(!_cpu->v86()) {
					...
					if(newcpl != _cpu->cpl()) {
						...
						void *tss;
						if(prepare_virtual(_cpu->tr.base + 4,8,MemTlb::TYPE_R,tss))
							return _fault;
						_cpu->efl &= ~(EFL_VM | EFL_TF | EFL_RF | EFL_NT);
						move<2>(&new_esp,tss);
						move<1>(&new_ss,reinterpret_cast<char *>(tss) + 4 * (newcpl + 1));
						_cpu->esp = new_esp;
						if(set_segment(&_cpu->ss,new_ss,false) || helper_PUSH<2>(&oldss)
								|| helper_PUSH<2>(&_oesp)) {
							_cpu->ss = oldss;
							return _fault;
						}
					...
					unsigned cs_sel = _cpu->cs.sel;
					if(helper_PUSH<2>(&old_efl) || helper_PUSH<2>(&cs_sel)
					...
					_cpu->esp = new_esp;
					unsigned cs_sel = _cpu->cs.sel;
					if(set_segment(&_cpu->ss,new_ss,false) || helper_PUSH<2>(&_cpu->gs.sel)
							|| helper_PUSH<2>(&_cpu->fs.sel) || helper_PUSH<2>(&_cpu->ds.sel)
							|| helper_PUSH<2>(&_cpu->es.sel) || helper_PUSH<2>(&old_ss)
							|| helper_PUSH<2>(&_oesp) || helper_PUSH<2>(&old_efl)
							|| helper_PUSH<2>(&cs_sel) || helper_PUSH<2>(&_cpu->eip)
							|| has_errorcode && helper_PUSH<2>(&error_code)
							|| set_segment(&_cpu->ds,0) || set_segment(&_cpu->es,0)
							|| set_segment(&_cpu->fs,0) || set_segment(&_cpu->gs,0)) {
...
int helper_INT(unsigned char vector) {
	return idt_traversal(0x80000600 | vector,0);
}
int helper_INVLPG() {
	return _fault;
}
...
int helper_MOV__EDX__DB0() {
	...
	switch(dbreg) {
		case 0 ... 3:
			...
			break;
		case 6:
			_dr6 = (value & ~0x1000) | 0xffff0ff0;
			break;
		case 7:
			_cpu->dr7 = (value & ~0xd800) | 0x400;
			break;
		default:
			UD0;
	}
...
int helper_FXSAVE() {
	...
	if(virt & 0xf)
		GP0; // could be also AC if enabled
	for(unsigned i = 0; i < sizeof(_fpustate) / sizeof(unsigned); i++) {
		void *addr;
		if(!virt_to_ptr(addr,4,user_access(TYPE_W),virt + i * sizeof(unsigned)))
			return _fault;
...
void helper_XLAT() {
	void *dst = 0;
	if(!logical_mem<0>((&_cpu->es) + ((_entry->prefixes >> 8) & 0xf),_cpu->ebx + _cpu->al,false,
			dst)) {
...
void helper_ENTER(unsigned *imm) {
	...
	if(helper_PUSH<operand_size>(&ebp))
		return;
	...
	for(unsigned n = 1; n < nesting_level; n++) {
		ebp -= 1 << operand_size;
		if(logical_mem<operand_size>(&_cpu->ss,ebp,false,tmp,true)
				|| helper_PUSH<operand_size>(tmp))
			return;
	}
	if(nesting_level && helper_PUSH<operand_size>(&frametemp))
		return;
	_cpu->esp -= (*imm & 0xffff);

	// check for errors while accessing the top of the stack
	if(logical_mem<operand_size>(&_cpu->ss,_cpu->esp,true,tmp,true))
		return;
