...
#define LOG if(_verbose) Serial::get().writef
...
class HostKeyboard : public StaticReceiver<HostKeyboard> {
	...
	unsigned char inb(unsigned short port) {
		...
		if(!_bus_hwioin.send(msg, true))
			Util::panic("%s could not read from ioport %x\n", __PRETTY_FUNCTION__, port);
		return msg.value;
	}
	...
	void outb(unsigned char value, unsigned short port) {
		...
		if(!_bus_hwioout.send(msg, true))
			Util::panic("%s could not send to ioport %x\n", __PRETTY_FUNCTION__, port);
	}
	...
	bool wait_output_full() {
		return wait_status(0x1, 1);
	}
	bool wait_input_empty() {
		return wait_status(0x2, 0);
	}
	...
	void handle_scancode(unsigned char key) {
		...
		if(_scset1) {
			...
			key = KeyboardUtil::translate_sc1_to_sc2(key);
		}
		...
		switch(key) {
			...
			case 0xF0:
				...
				return;
			case 0x11: // alt
				nflag = (_flags & Keyboard::EXTEND0) ? Keyboard::RALT : Keyboard::LALT;
				break;
			case 0x12: // lshift
			case 0x59: // rshift
				...
				nflag = key == 0x12 ? Keyboard::LSHIFT : Keyboard::RSHIFT;
				break;
			case 0x14: // ctrl
				nflag = (_flags & Keyboard::EXTEND0) ? Keyboard::RCTRL : Keyboard::LCTRL;
				break;
			case 0x1f:
				nflag = (_flags & Keyboard::EXTEND0) ? Keyboard::LWIN : 0;
				break;
			case 0x27:
				nflag = (_flags & Keyboard::EXTEND0) ? Keyboard::RWIN : 0;
				break;
			case KeyboardUtil::KBCODE_NUM:
		...
		if((_flags & Keyboard::EXTEND1) &&
		   ((key == 0x14) || (key == 0x77 && (~_flags & Keyboard::RELEASE))))
			return;
...
public:
	...
	HostKeyboard(DBus<MessageHwIOIn> &bus_hwioin, DBus<MessageHwIOOut> &bus_hwioout,
	             DBus<MessageInput> &bus_input, unsigned hostdev, unsigned short base,
    ...
    "A missing auxirq omits the mouse initialisation. ")
{
	...
	uint16_t hostiobase = ~argv[1] ? argv[1] : 0x60;
	...
	if(!mb.bus_hostop.send(msg1) || !mb.bus_hostop.send(msg2))
		Util::panic("%s failed to allocate ports %x, %x\n", __PRETTY_FUNCTION__, hostiobase,
		            hostiobase + 4);

	HostKeyboard *dev = new HostKeyboard(mb.bus_hwioin, mb.bus_hwioout, mb.bus_input, hdev, hostiobase,
	                                     kbirq, auxirq, argv[4], ~argv[5] ? argv[5] : 0);
	...
	MessageHostOp msg3 = MessageHostOp::attach_irq(kbirq, -1, true, "keyb");
	MessageHostOp msg4 = MessageHostOp::attach_irq(auxirq, -1, true, "aux");
	if(!(msg3.value == ~0U ||
	     mb.bus_hostop.send(msg3)) || !(msg4.value == ~0U || mb.bus_hostop.send(msg4)))
		Util::panic("%s failed to attach hostirq %x, %x\n", __PRETTY_FUNCTION__, kbirq, auxirq);
}
