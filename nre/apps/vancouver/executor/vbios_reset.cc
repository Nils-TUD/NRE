...
#define ACPI_MANUFACTURER	"bk@vmmon"
	...
	struct Resource {
		...
		Resource(const char *_name,size_t _offset,size_t _length,bool _acpi_table) :
				name(_name), offset(_offset), length(_length), acpi_table(_acpi_table) {
		}
	...
	bool reset_helper(MessageBios &msg) {
		...
		state->edx = 0;
		state->ecx = 0x1b;
		CpuMessage msg1(CpuMessage::TYPE_WRMSR,state,Mtd::GPR_ACDB);
		...
		for(size_t j = 0; j < ARRAY_SIZE(msg2); j++)
			vcpu->mem.send(msg2[j],true);
		...
		check1(false,!_mb.bus_memregion.send(msg3) || !msg3.ptr || !msg3.count,
				"no low memory available");
		...
		discovery_write_dw("bda",0x13,static_cast<unsigned>(_mem_size >> 10),2);
	...
	Resource * get_resource(const char *name) {
		for(size_t i = 0; i < MAX_RESOURCES; i++) {
			...
			if(!strcmp(_resources[i].name,name))
				return _resources + i;
		}
		return 0;
	}
	...
	unsigned acpi_tablesize(Resource *r) {
		return *reinterpret_cast<unsigned *>(_mem_ptr + r->offset + 4);
	}
	...
	bool create_resource(size_t index,const char *name) {
...
public:
	bool receive(MessageBios &msg) {
		switch(msg.irq) {
			...
			case 0x18:
				Serial::get().writef("INT18 - new try\n");
				return jmp_int(msg,0x19);
			default:
				return false;
		}
		...
		switch(msg.type) {
			case MessageDiscovery::WRITE: {
				...
				check1(false,needed_len > r->length,
						"WRITE no idea how to increase the table %s size from %d to %d",msg.resource,r->length,needed_len);
			...
			case MessageDiscovery::READ: {
				...
				check1(false,!(r = get_resource(msg.resource)));
				check1(false,needed_len > r->length,
						"READ no idea how to increase the table %s size from %d to %d",msg.resource,r->length,needed_len);
