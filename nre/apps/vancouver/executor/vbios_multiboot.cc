...
private:
	...
	unsigned long init_mbi(uintptr_t &rip) {
		...
		for(size_t modcount = 0;; modcount++) {
			...
			switch(modcount) {
				case 0:
					if(Elf::decode_elf(msg2.start,msg2.size,physmem,rip,offset,memsize,0,0))
						return 0;
					offset = (offset + 0xfff) & ~0xffful;
					mbi = offset;
					offset += 0x1000;
				...
				default: {
					...
					Module *mod = reinterpret_cast<Module *>(physmem + m->mods_addr)
							+ m->mods_count;
					m->mods_count++;
					...
					if(offset < mod->mod_end)
						offset = mod->mod_end;
					if(offset < mod->string + msg2.cmdlen)
						offset = mod->string + msg2.cmdlen;
				}
				break;
			}
		}
...
public:
	bool receive(MessageBios &msg) {
		...
		msg.cpu->tr.ar = 0x8b;
		msg.cpu->ss.base = msg.cpu->ds.base = msg.cpu->es.base = msg.cpu->fs.base =
				msg.cpu->gs.base = msg.cpu->cs.base;
		msg.cpu->ss.limit = msg.cpu->ds.limit = msg.cpu->es.limit = msg.cpu->fs.limit =
				msg.cpu->gs.limit = msg.cpu->cs.limit;
		msg.cpu->tr.limit = msg.cpu->ld.limit = msg.cpu->gd.limit = msg.cpu->id.limit = 0xffff;
		msg.mtr_out = Mtd::ALL & ~Mtd::TSC;
		return true;
	}
...
PARAM_HANDLER(vbios_multiboot,
	...
	mb.bus_bios.add(
			new VirtualBiosMultiboot(mb,argv[0] != ~0ul ? argv[0] : _vbios_multiboot_modaddr,
					argv[1] != ~0ul ? argv[1] : 0xa0000),VirtualBiosMultiboot::receive_static);
}
