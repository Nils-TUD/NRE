 ...
 */
enum {
	FAULT_NOERROR, FAULT_RETRY, FAULT_RECALL, FAULT_UNIMPLEMENTED,
};
...
public:
	...
	unsigned modrm2virt() {
		...
		if(info & MRM_SIB) {
			...
			if(~info & MRM_NOBASE)
				virt += _cpu->gpr[info & 0x7];
			if(~info & MRM_NOINDEX)
				virt += _cpu->gpr[(info >> 3) & 0x7] << ((info >> 6) & 0x3);
			disp_offset++;
		}
		else {
			if((info & 0xf) || (info & MRM_EAX))
				virt += _cpu->gpr[info & 0x7];
			if(info & 0xf0)
				virt += _cpu->gpr[(info >> 4) & 0x7];
		}

		unsigned disp = ((info >> MRM_DISSHIFT) & 0x3);
		switch(disp) {
			case 1:
				virt += *reinterpret_cast<char *>(disp_offset);
				break;
			case 2:
				virt += *reinterpret_cast<short *>(disp_offset);
				break;
			case 3:
				virt += *reinterpret_cast<int *>(disp_offset);
				break;
			default:
				break;
		}
...
#    define CLOBBER      "memory"
	...
	int execute() {
		...
		Type type = TYPE_R;
		if(!(_entry->flags & (IC_DIRECTION | IC_READONLY)))
			type = TYPE_W;
