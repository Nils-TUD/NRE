...
class InstructionCache : public MemTlb
{
  ...
  int event_injection()
  {
    if (_mtr_in & nul::Mtd::INJ && _cpu->inj_info & 0x80000000 && !idt_traversal(_cpu->inj_info, _cpu->inj_error)) {
  ...
  {
    unsigned virt = READ(rip) + entry->inst_len;
  ...
  bool find_entry(unsigned &index)
  {
    ...
    unsigned linear = _cpu->rip + READ(cs).base;
...
public:
  ...
  int get_instruction()
  {
    ...
    _entry = _values + index;
    _cpu->rip += _entry->inst_len;
    if (debug) {
    	nul::Serial::get().writef("eip %x:%x esp %x eax %x ebp %x prefix %x\n", _cpu->cs.sel, _oeip, _oesp, _cpu->rax, _cpu->rbp, _entry->prefixes);
  ...
  void call_asm(void *tmp_src, void *tmp_dst)
  {
	...
	_cpu->rfl = (_cpu->rfl & ~0x8d5) | (tmp_flag  & 0x8d5);
	_mtr_out |= nul::Mtd::RFLAGS;
	break;
      case IC_LOADFLAGS:
	tmp_flag = _cpu->rfl & 0x8d5;
  ...
  bool commit()
  {
	...
	if (_cpu->rsp != _oesp) _mtr_out |= nul::Mtd::RSP;
	...
	if (~_fault & 0x80000000)
	...
	else
	  {
	    ...
	    nul::Serial::get().writef("fault: %x old %x error %x cr2 %x at eip %x line %d %x\n", _fault, _cpu->inj_info,
			    _error_code, _cpu->cr2, _cpu->rip, _debug_fault_line, _cpu->cr2);
