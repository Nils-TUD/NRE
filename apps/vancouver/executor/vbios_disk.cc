...
class VirtualBiosDisk : public StaticReceiver<VirtualBiosDisk>, public BiosCommon
{
  ...
  {
    Serial::get().writef("boot from disk\n");
    msg.cpu->ss.sel  = 0;
    msg.cpu->ss.base = 0;
    msg.cpu->esp     = 0x7000;
  ...
  bool handle_int13(MessageBios &msg)
  {
	...
	if (check_drive(msg, disk_nr))
	  {
	    ...
	    params.pheads = 255;
	    params.psectors = 63;
	    unsigned long long sectors =  _disk_params[disk_nr].sectors / (params.psectors * params.pheads);
    ...
    msg.mtr_out |= Mtd::GPR_ACDB | Mtd::RFLAGS;
    return true;
  }
