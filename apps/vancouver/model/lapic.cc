...
private:
  ...
  unsigned get_highest_bit(unsigned bit_offset) {
    for (int i=7; i >=0; i--) {
      ...
      if (value) return (i << 5) | Math::bit_scan_reverse(value);
    }
    return 0;
  }
  ...
  void broadcast_eoi(unsigned vector) {
    if (!get_bit(_vector, OFS_TMR + _isrv)) return;
  ...
  bool register_write(unsigned offset, unsigned value, bool strict) {
	...
	if (_isrv == (_TIMER & 0xff)) update_timer(_mb.clock().source_time());
	...
	if (offset != 3)  Serial::get().writef("LAPIC write %x at offset %x failed\n", value, offset);
	set_error(7);
      }
    }
    ...
    if (in_range(offset, LVT_BASE, NUM_LVT)) {
      ...
      if (offset == _TIMER_offset)   update_timer(_mb.clock().source_time());
      update_irqs();
    }
    return res;
  }
...
public:
  ...
  bool  receive(CpuMessage &msg) {
    if (msg.type == CpuMessage::TYPE_RDMSR) {
      ...
      if (!register_read(msg.cpu->ecx & 0x3f, eax)) return false;
      msg.cpu->eax = eax;
...
{
  if (!mb.last_vcpu) Util::panic("no VCPU for this APIC");
