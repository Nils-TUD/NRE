...
#include "reg.h"
  ...
  bool handle_cpuid(CpuMessage &msg) {
    ...
    unsigned val;
    if (!CPUID_read(reg | 0, val)) msg.cpu->eax = 0; else msg.cpu->eax = val;
    if (!CPUID_read(reg | 1, val)) msg.cpu->ebx = 0; else msg.cpu->ebx = val;
    if (!CPUID_read(reg | 2, val)) msg.cpu->ecx = 0; else msg.cpu->ecx = val;
    if (!CPUID_read(reg | 3, val)) msg.cpu->edx = 0; else msg.cpu->edx = val;
  ...
  void handle_rdtsc(CpuMessage &msg) {
    assert((msg.mtr_in & Mtd::TSC) and (msg.mtr_in & Mtd::GPR_ACDB));
    msg.cpu->edx_eax(msg.cpu->tsc_off + Util::tsc());
    msg.mtr_out |= Mtd::GPR_ACDB;
  }
  ...
  void handle_rdmsr(CpuMessage &msg) {
    ...
    case 0x10:
      ...
      break;
    case 0x174 ... 0x176:
      assert(msg.mtr_in & Mtd::SYSENTER);
  ...
  void handle_wrmsr(CpuMessage &msg) {
    ...
    switch (cpu->ecx)
      {
      case 0x10:
	assert(msg.mtr_in & Mtd::TSC);
	cpu->tsc_off = - Util::tsc() + cpu->edx_eax() - cpu->tsc_off;
	msg.mtr_out |= Mtd::TSC;
	break;
      case 0x174 ... 0x176:
  ...
  void prioritize_events(CpuMessage &msg) {
    ...
    assert(msg.mtr_in & Mtd::INJ);
    assert(msg.mtr_in & Mtd::RFLAGS);
    msg.mtr_out |= Mtd::STATE | Mtd::INJ;
  ...
  void got_event(unsigned value) {
    ...
    if (value & DEASS_INTR) Atomic::bit_and<volatile unsigned>(&_event, ~EVENT_INTR);
    ...
    if ((value & EVENT_MASK) == EVENT_SIPI)
       ...
       */
      if (Atomic::cmpxchg4b(&_sipi, 0, value)) return;

    Atomic::bit_or<volatile unsigned>(&_event, STATE_WAKEUP | (value & (EVENT_MASK | EVENT_DEBUG | EVENT_HOST)));
...
public:
  ...
  bool receive(CpuMessage &msg) {
    ...
    if (msg.type != CpuMessage::TYPE_CPUID_WRITE && msg.mtr_in & Mtd::TSC && ~msg.mtr_out & Mtd::TSC) {
  ...
  VirtualCpu(VCpu *_last, Motherboard &mb) : VCpu(_last), _mb(mb), _event(0), _sipi(~0u)  {
    ...
    if (!mb.bus_hostop.send(msg)) Util::panic("could not create VCpu backend.");
