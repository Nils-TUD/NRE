...
class KeyboardController: public StaticReceiver<KeyboardController> {
	...
	void read_all_devices() {
		if(_ram[RAM_LOCK])
			return;
		_ram[RAM_LOCK] = 1;
		if(~_ram[RAM_CMDBYTE] & CMD_DISAUX)
			read_from_device(_ps2ports + 1);
		if(~_ram[RAM_CMDBYTE] & CMD_DISKBD)
			read_from_device(_ps2ports);
		_ram[RAM_LOCK] = 0;
	}
...
public:
				...
				switch(_ram[RAM_LASTCMD]) {
					...
					case 0xa5: // load pwd
						if(_ram[RAM_PWD_COUNT] + RAM_PWD_FIRST <= RAM_PWD_LAST) {
							...
							_ram[RAM_PWD_COUNT]++;
							if(!msg.value)
								_ram[RAM_PWD_COUNT] = RAM_PWD_LAST + 1;
						}
						break;
	...
	KeyboardController(DBus<MessageIrqLines> &bus_irqlines,DBus<MessagePS2> &bus_ps2,
			...
			unsigned ps2ports) :
			_bus_irqlines(bus_irqlines), _bus_ps2(bus_ps2), _bus_legacy(bus_legacy), _base(base), _irqkbd(
					irqkbd), _irqaux(irqaux), _ps2ports(ps2ports) {
	}
};
