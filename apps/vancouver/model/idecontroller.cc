...
#include "model/reg.h"
 ...
 public:
  ...
  bool  receive(MessageIOIn &msg)
  {
	...
	if (!_bufferoffset) Logging::printf("data[%d] = %04x\n", _bufferoffset, msg.value);
  ...
  bool  receive(MessageIOOut &msg)
  {
	...
	return true;
      case 7:
	_command = msg.value;
	Logging::printf("issue command %x\n", _command);
	issue_command(true);
	return true;
      }
    }
      ...
      _irq(irq), _bdf(bdf), _disknr(disknr), _params(params), _buffer(buffer), _baddr(baddr), _bufferoffset(0)
  {
    PCI_reset();
    reset_device();
    Logging::printf("instanciate IDE controller with bdf %x for disk '%s' sectors %llx\n", bdf, params.name, params.sectors);
  }
};
