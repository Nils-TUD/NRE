...
public:
	bool receive(MessageIrq &msg) {
		if(msg.line == _host_irq) {
			...
			if(_msi_vector >= 0x10 && _msi_vector < 0x100)
				_mb.bus_mem.send(msg1);
...
PARAM_HANDLER(hostirq,
	...
	mb.bus_hostirq.add(new IRQRouting(mb,argv[0],argv[1],argv[2]),
			IRQRouting::receive_static<MessageIrq>);
	MessageHostOp msg(MessageHostOp::OP_ATTACH_IRQ,argv[0]);
	if(!mb.bus_hostop.send(msg))
		Util::panic("%s failed to attach hostirq %lx\n",__PRETTY_FUNCTION__,msg.value);
}
