...
public:
	...
	timevalue_t timestamp() {
		...
		if(~data[0xb] & 2) {
			...
			if(~data[0xb] & 4)
				hour = nul::Math::from_bcd(hour);
			hour %= 12;
			if(data[4] & 0x80)
				hour += 12;
			if(~data[0xb] & 4)
				hour = nul::Math::to_bcd(hour);
			data[4] = hour;
		}
		...
		if(~data[0xb] & 4) {
			for(size_t i = 0; i < sizeof(data) && i < 10; i++)
				data[i] = nul::Math::from_bcd(data[i]);
		}
		...
		if(year < 1970)
			year += 100;
		nul::DateInfo date(year,data[8],data[7],data[4],data[2],data[0]);
		return nul::Date::mktime(&date) * nul::Timer::WALLCLOCK_FREQ;
	}
